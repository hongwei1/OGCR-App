...
export const actions: Actions = {
	default: async ({ request, locals, params }) => {
		...
		const values: Record<string, string> = {
			trade_id: (formData.get('trade_id') as string) ?? '',
		...
		if (values.step) body.step = values.step;

		try {
			const response = await obp_requests.post(tradingPaths.settlements(ctx), body, accessToken);
